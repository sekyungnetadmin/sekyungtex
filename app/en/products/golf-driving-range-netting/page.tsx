...
const features = [
  'Knotless construction — smooth surface, reduced ball damage',
  'UV-stabilized yarn — long outdoor lifespan',
  'CAD-based square-cut fabrication for tower structures',
  'Custom mesh size and net dimensions available',
  'KS / ISO certified — test reports issued on request',
]

const specs = [
  { label: 'Material',      value: 'HDPE / Polyester / Dyneema' },
  { label: 'Construction',  value: 'Knotless' },
  { label: 'Mesh size',     value: '43mm – 50mm (custom)' },
  { label: 'Color',         value: 'Green, Black, White, Brown, Grey' },
  { label: 'Size',          value: 'Custom cut to order' },
  { label: 'Lead time',     value: '1–3 weeks' },
  { label: 'Certification', value: 'KS, ISO, UV resistance test' },
]

const materials = [
  {
    name: 'HDPE (High-Density Polyethylene)', tag: 'Most Popular', tagColor: 'bg-[#e8c840] text-[#111]',
    desc: 'The standard choice for golf driving ranges across Asia. Lightweight, UV-resistant, and cost-effective for everyday use at driving ranges and practice facilities.',
    props: ['Lightweight','UV-resistant','High impact resistance','Wide mesh range'],
    use: 'Driving ranges · Practice facilities · Golf academies',
  },
  {
    name: 'Polyester', tag: '', tagColor: '',
    desc: 'High-tenacity polyester offers excellent dimensional stability and low stretch. A reliable choice for facilities requiring consistent net geometry over time.',
    props: ['Dimensionally stable','Low stretch','Good UV resistance','Durable'],
    use: 'Driving ranges · Indoor facilities',
  },
  {
    name: 'Dyneema (UHMWPE)', tag: '', tagColor: '',
    desc: 'Ultra-high-strength fiber used in professional sports facilities. 15× stronger than steel by weight — virtually invisible on-site.',
    props: ['Extreme tensile strength','Ultra-thin diameter','Near-transparent','Long service life'],
    use: 'Pro facilities · High-impact applications',
  },
]

const faqs = [
  { q: 'Can you manufacture custom sizes?', a: 'Yes. All nets are custom-fabricated based on your site dimensions and tower drawings.' },
  { q: 'What is the lead time?', a: 'Standard orders are fulfilled within 1–3 weeks. Large custom orders may vary.' },
  { q: 'Do you provide installation support?', a: 'Installation support is available for indoor golf practice ranges. For overseas orders, availability depends on order volume — please contact us for details.' },
]
...
const whyUs = [
  { title: '35+ Years Experience', sub: 'Proven industrial netting manufacturer' },
  { title: 'In-house Production',  sub: 'Full control from yarn to finished net' },
  { title: 'Custom Every Order',   sub: 'CAD-based cut to your exact spec' },
]
    ...
    <aside className="bg-white">
      <div className="p-4 border-b border-gray-100">
        ...
        <div className="mt-4 flex flex-col gap-2">
          {[{l:'TEL',v:'+82-051-262-5631'},{l:'Email',v:'info@sekyungtex.co.kr'},{l:'Chat',v:'WhatsApp available'}].map((r)=>(
            <div key={r.l} className="flex gap-2">
              <span className="text-[11px] text-gray-300 min-w-[36px] pt-px">{r.l}</span>
              <span className="text-[12px] text-gray-600">{r.v}</span>
            </div>
          ))}
        </div>
      </div>
      <div className="p-4 border-b border-gray-100">
        ...
        <div className="flex flex-col gap-1.5">
          {otherProducts.map((p)=>(
            <a key={p.name} href={p.href} className={`flex items-center justify-between px-3 py-2.5 rounded-md border text-[12px] transition-colors ${p.active?'bg-[#1a2332] border-[#1a2332] text-[#e8c840] font-medium':'bg-[#f8f8f6] border-[#ebebeb] text-gray-600 hover:bg-gray-100'}`}>
              <span>{p.name}</span><span className={p.active?'text-[#e8c840]':'text-gray-300'}>›</span>
            </a>
          ))}
        </div>
      </div>
      <div className="p-4">
        ...
        <div className="bg-[#1a2332] rounded-lg p-4">
          {whyUs.map((item)=>(
            <div key={item.title} className="flex gap-2.5 mb-3 last:mb-0">
              <div className="w-7 h-7 bg-[#e8c840]/15 rounded flex items-center justify-center flex-shrink-0">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="#e8c840" strokeWidth="2"><polyline points="2,7 5,10 12,3"/></svg>
              </div>
              <div>
                <p className="text-[12px] font-medium text-white">{item.title}</p>
                <p className="text-[11px] text-[#7a93ab] leading-snug">{item.sub}</p>
              </div>
            </div>
          ))}
        </div>
...
export default function GolfNettingPage() {
  return (
    <main className="bg-white text-sm">
      <section className="bg-[#1a2332] w-full relative overflow-hidden px-6 py-14" style={{backgroundImage:`linear-gradient(rgba(26,35,50,0.82),rgba(26,35,50,0.82)),url(/assets/sample-golf.png)`,backgroundSize:'cover',backgroundPosition:'center'}}>
        ...
        <div className="relative max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-[1fr_300px] gap-8 md:gap-12 items-center">
          <div>
            ...
            <p className="text-[14px] text-[#8fa3bb] leading-relaxed mb-6 max-w-xl">
              High-tensile <strong className="text-white font-medium">knotless</strong> golf barrier nets manufactured in Korea. HDPE, Polyester, and Dyneema options — lightweight and long-lasting.
            </p>
          ...
          <div className="hidden md:block bg-white/5 border border-white/10 rounded-xl p-5">
            ...
            {[{l:'Construction',v:'Knotless'},{l:'Material',v:'HDPE / Polyester / Dyneema'},{l:'Lead time',v:'1–3 weeks'},{l:'Certification',v:'KS · ISO'}].map((row)=>(
              <div key={row.l} className="flex justify-between py-2 border-b border-white/[0.07] last:border-0">
                <span className="text-[12px] text-[#8fa3bb]">{row.l}</span>
                <span className="text-[12px] font-semibold text-white">{row.v}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white w-full border-b border-gray-200 px-6 py-2.5">
        <nav className="max-w-7xl mx-auto flex gap-1.5 text-[12px] text-gray-400">
          ...
          <span className="text-gray-900">Golf Driving Range Netting</span>
        </nav>
      </section>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-[1fr_300px]">
          <div className="border-r-0 lg:border-r border-gray-200">
            ...
            <div className="bg-[#1a2332] px-8 py-7 border-b border-[#243040]">
              ...
              <p className="text-[13px] text-[#8fa3bb] leading-relaxed mb-5">
                Knotless nets are the industry standard for golf driving ranges and sports facilities across Asia. Without knots, the surface is smooth and even — reducing wear on balls, lowering wind resistance, and extending the lifespan of the net.
              </p>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white/5 border border-white/10 rounded-lg p-4">
                  ...
                  {['Smooth surface — less ball wear','Lighter weight, less wind load','No weak points at knots','Longer service life','Cleaner appearance on-site'].map((t)=>(
                    <div key={t} className="flex items-start gap-2 mb-1.5 last:mb-0">
                      <span className="text-[#e8c840] text-[11px] mt-0.5">✓</span>
                      <span className="text-[12px] text-[#c8dce8]">{t}</span>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white px-8 py-6 border-b border-gray-100">
              <div className="relative w-full h-64 rounded-lg overflow-hidden">
                <Image src="/assets/sports1.jpg" alt="Golf driving range barrier net by SekyungNET" fill className="object-cover"/>
              </div>
            </div>
                ...
                {materials.map((mat)=>(
                  <div key={mat.name} className="border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {mat.props.map((p)=><span key={p} className="text-[11px] bg-[#f8f8f6] border border-[#ebebeb] text-gray-600 px-2.5 py-0.5 rounded-full">{p}</span>)}
                    </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {applications.map((app)=>(
                  <div key={app} className="flex items-center gap-2.5 px-3.5 py-3 rounded-md bg-[#f8f8f6] border border-[#ebebeb]">
                    <div className="w-4 h-0.5 bg-[#e8c840] flex-shrink-0 rounded-sm"/>
              ...
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                <table className="w-full text-[13px]">
                  <thead><tr className="bg-[#1a2332]">
                    <th className="text-left px-4 py-2.5 text-[#8fa3bb] font-medium text-[11px] tracking-wider uppercase">Item</th>
                    <th className="text-left px-4 py-2.5 text-[#8fa3bb] font-medium text-[11px] tracking-wider uppercase">Detail</th>
                  </tr></thead>
                  <tbody>{specs.map((row)=>(
                    <tr key={row.label} className="border-t border-gray-100">
                      <td className="px-4 py-3 text-gray-500 w-[38%]">{row.label}</td>
